...
class NumberUtil {
  ...
  getHeightByWidth = function (newWidthNum, curWidth, curHeight) {
  ...
  getDestinationVector = function (originVector, angle, distance) {
  ...
  getDistanceByPercent = function (totalDistance, percent) {
    var curDist = (totalDistance * percent) / 100;

    return curDist;
  };

  getPercent = function (curTotal, total) {
    var percent = Math.floor((curTotal * 100) / total);

    return percent;
  };

  getHypotenuse = function (width, height) {
    return Math.sqrt(Math.pow(width, 2) + Math.pow(height, 2));
  };

  getMoveVector = function (angle, distance) {
    ...
    var moveVector = scratch.vector().set(distance * Math.cos(angle), distance * Math.sin(angle));
  ...
  getRandomMoveVector = function () {
    ...
    var amount = 0.000075;
    // point the acceleration in the direction of the ship's nose
    var moveVector = scratch.vector().set(amount * Math.cos(angle), amount * Math.sin(angle));
