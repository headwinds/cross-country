...
const GoldLeafImage = ({ goldLeafModel, onNoImageFoundCallback }: GoldLeafImageProps) => {
  ...
  return (
    <Column customClass={styles.GoldLeaf__image}>
      <Image url={goldLeafModel?.images?.[0].imageUrl ?? ''} a11y={goldLeafModel?.title ?? ''} onNoImageFoundCallback={onNoImageFoundCallback} />
    </Column>
  );
};
