...
const url = screenDocument?.domain?.includes("localhost")
  ? localUrl
  : remoteUrl;
...
const fetchMap = (context, event) => {
  ...
  fetch(`${MAP_URL}`)
    .then((response) => {
      console.log(
        "Cross Country: cave-troll-machine fetchMap ...heard response: ",
        response
      );
      if (response.ok) {
    ...
    .then((data) => {
      console.log(
        "Cross Country: cave-troll-machine fetchMap ...json data: ",
        data
      );
      return data;
    })
...
const updateTileModelsAfterMapGenerated = (context, event) => {
  ...
  console.log(
    "updateTileModelsAfterMapGenerated here updatedTileModels: ",
    updatedTileModels
  );
    ...
    invoke: {
      ...
      src: (context, event) => (callback, onEvent) => {
        ...
        const onGetMapSuccessHandler = (data) => {
          console.log(
            "Cross Country: cave-troll-machine onGetMapSuccessHandler called...",
            data
          );
          const {
            result: { map },
