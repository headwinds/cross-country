export function email(branch, email){
    ...
    const via = "\n\n\n\n\n via porthole";
    body += via;
    const emailLoc = "mailto:"+emailTo+"?subject="+encodeURIComponent(subject)+"&body="+encodeURIComponent(body);
    window.open(emailLoc, "_blank", 'noopener'); 
}
export function tweet(branch){
    ...
    window.open(statusWindowURL, 'twitterwindow', 'height=380, width=550, top='+(window.innerHeight/2 - 225) +', left='+ window.innerWidth/2 +', toolbar=0, location=0, menubar=0, directories=0, scrollbars=0');
